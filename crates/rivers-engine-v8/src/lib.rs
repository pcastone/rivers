...
thread_local! {
    ...
    static TASK_APP_ID: RefCell<Option<String>> = RefCell::new(None);

    /// Node ID.
    static TASK_NODE_ID: RefCell<Option<String>> = RefCell::new(None);

    /// Runtime env.
    static TASK_RUNTIME_ENV: RefCell<Option<String>> = RefCell::new(None);
}

/// Set up thread-locals from a serialized task context.
fn setup_task_locals(ctx: &SerializedTaskContext) {
...
static SCRIPT_CACHE: std::sync::LazyLock<StdMutex<HashMap<String, String>>> =
    std::sync::LazyLock::new(|| StdMutex::new(HashMap::new()));

// ── Isolate Pool ────────────────────────────────────────────────

const DEFAULT_HEAP_LIMIT: usize = 128 * 1024 * 1024;
...
fn v8_to_json_value(scope: &mut v8::HandleScope, val: v8::Local<v8::Value>) -> serde_json::Value {
...
fn execute_js(ctx: SerializedTaskContext) -> Result<SerializedTaskResult, String> {
