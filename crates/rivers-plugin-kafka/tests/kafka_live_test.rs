...
#[tokio::test]
async fn kafka_contract_ack_returns_acked() {
    test_ack_returns_acked(&KafkaDriver).await;
}

#[tokio::test]
async fn kafka_contract_nack_redelivery_or_unsupported() {
    test_nack_redelivery_or_unsupported(&KafkaDriver).await;
}

#[tokio::test]
async fn kafka_contract_consumer_group_exclusive() {
    test_consumer_group_exclusive(&KafkaDriver).await;
}

#[tokio::test]
async fn kafka_contract_multi_subscription() {
    test_multi_subscription(&KafkaDriver).await;
}
