...
| `crates/rivers-runtime/src/pool_handle.rs` | `ReleaseToken::release` made async to guarantee in-line pool return. | `docs/code_review.md` P0-3 | Added `#[async_trait]` to the trait; `self: Box<Self>` receiver supported by async-trait 0.1.x. |
| `crates/riversd/src/pool_handle_impl.rs` | Drop `tokio::spawn` and await `pool.release` directly. | `docs/code_review.md` P0-3 | Caller is always async; `tokio::spawn` was the only reason the previous sync trait method existed. |
| `crates/rivers-runtime/src/dataview_engine.rs` | Await `release_token.release(conn)` in `execute` and `execute_ddl`. | `docs/code_review.md` P0-3 | Mechanical `.await` addition at both call sites. |

# 2026-04-25 — DataView ↔ PoolManager wiring (CR-P0-3)

| File | Summary | Reference | Resolution |
|------|---------|-----------|------------|
| `crates/rivers-runtime/src/pool_handle.rs` | New module — defines the `PoolManagerHandle` trait (dyn-safe `acquire` + `snapshots`) plus the `PoolAcquireToken` / `ReleaseToken` types used by `DataViewExecutor` to obtain pooled connections without depending on `riversd::pool`. | `docs/code_review.md` finding P0-3; `rivers-data-layer-spec.md §5` | Trait kept minimal so the `rivers-runtime → riversd` dependency direction is preserved; concrete impl lives in `riversd`. |
| `crates/riversd/src/pool.rs` | Added `PoolManager::ensure_pool(datasource_id, …)` — idempotent registration of a `ConnectionPool` per datasource at bundle load. | `docs/code_review.md` P0-3 | Existing `register_pool` was insertion-only; `ensure_pool` is the load-path entry point that returns the already-registered pool when present, building one only on first call. |
| `crates/riversd/src/pool_handle_impl.rs` | New bridge module — implements `PoolManagerHandle` for `PoolManager`, exposed by adding `mod pool_handle_impl;` in `lib.rs`. | `docs/code_review.md` P0-3 | Single-file bridge: keeps the trait surface in `rivers-runtime` while the concrete pool stays in `riversd`. |
| `crates/riversd/src/server/context.rs` | Added `pool_manager: Arc<PoolManager>` field to `AppContext` so handlers, health endpoints, and bundle loading can route through the pool. | `docs/code_review.md` P0-3 | Held as the concrete `Arc<PoolManager>` (not the trait object) so non-handler code can call inherent APIs (`ensure_pool`, `snapshots`) directly. |
| `crates/riversd/src/bundle_loader/load.rs` | At app load, register one `ConnectionPool` per database datasource via `pool_manager.ensure_pool(...)` (idempotent, skips message-broker datasources); inject `Arc<dyn PoolManagerHandle>` into the `DataViewExecutor` for that bundle. | `docs/code_review.md` P0-3; `rivers-data-layer-spec.md §5` | Pool registration is per-datasource, runs once at load, and uses the existing `DriverFactory`. The handle is coerced via `as Arc<dyn PoolManagerHandle>` at the wiring site. |
| `crates/rivers-runtime/src/dataview_engine.rs` | `DataViewExecutor::execute(...)` now acquires a connection via the injected `PoolManagerHandle` instead of calling `factory.connect(...)` directly. Message-broker datasources continue to fall through to the broker dispatch path unchanged. | `docs/code_review.md` P0-3 | Acquire → run query → `release_token.release(conn).await`. Broker fallthrough preserved by short-circuiting before the pool acquire when the datasource type is a message broker. |
| `crates/rivers-runtime/src/dataview_engine.rs` | `DataViewExecutor::execute_ddl(...)` routes through the pool on the same `PoolManagerHandle`. **Caveat: dormant in production today** — the runtime DDL host callback `host_ddl_execute` (`crates/riversd/src/engine_loader/host_callbacks.rs`) bypasses `execute_ddl` and calls `factory.connect` directly. The pool routing fires only via direct `DataViewExecutor::execute_ddl` callers (tests). Captured as follow-up `CR-P0-3.follow` in `todo/tasks.md`. | `docs/code_review.md` P0-3 | Pool wiring on the DDL path is in place and unit-tested; production dispatch route fix tracked separately. |
| `crates/riversd/src/server/handlers.rs` | Verbose health endpoint now populates `pool_snapshots` from `AppContext.pool_manager.snapshots()` instead of returning an empty list. | `docs/code_review.md` P0-3 ("Verbose health endpoint reports a non-empty pool snapshot for the active datasource") | Operator-facing snapshot rows are sourced directly from per-datasource `ConnectionPool` state. |
| `crates/riversd/src/pool.rs` | `PoolSnapshot` gained `driver_name: String` and `circuit_state: String` fields so `health::PoolSnapshot.driver` and `circuit_state` columns have real data instead of placeholders. | `docs/code_review.md` P0-3 | Source-of-truth shift: snapshot now carries enough metadata for the verbose health view without re-walking the registry. |
| `crates/riversd/tests/dataview_pool_integration_tests.rs` | New integration tests: 50 sequential DataView calls reuse a single connection (perfect reuse via async release); pool eviction occurs when `max_lifetime_ms` elapses; broker datasources fall through to the broker path without pool involvement. | TDD discipline for CR-P0-3 | Tests use `MockDriver`-style counters to assert reuse and eviction observably; broker test asserts no pool acquire is recorded. |

**Canary regression (plan Task 10):** deferred to a separate regression session per user direction. Per-crate test sweeps verified during implementation: 28/28 `pool_tests`, 3/3 `dataview_pool_integration_tests`, 5/5 `ddl_pipeline_tests`, 58/59 `dataview_engine_tests` (1 pre-existing flake unrelated to this work).
| `crates/riversd/tests/dataview_pool_integration_tests.rs` | Removed `tokio::task::yield_now()` workaround in reuse test; tightened assertion to `connect_count == 1` for 50 sequential calls. | `docs/code_review.md` P0-3 | Async release now lands the connection in idle before next acquire, so reuse is deterministic. |
