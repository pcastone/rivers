...
All declared stages execute in order (or concurrently if `parallel = true`). Each deposits data into `ctx.sources[key]`. No short-circuit — a failing stage deposits nothing and logs, but subsequent stages still run. Typical uses: fetching foreign data (pricing, inventory, permissions) that will be merged with primary results.
...
Contiguous `on_request` or `on_response` stages marked `parallel = true` are collected and executed via `join_all`. Sequential stages see results from all prior stages. Parallel stages share a snapshot of `ctx` at the point they start — they do not see each other's deposits.

```toml
on_request = [
    { module = "h/pricing.ts",   entrypoint = "fetch", key = "pricing",   parallel = true },
    { module = "h/inventory.ts", entrypoint = "fetch", key = "inventory", parallel = true },
    { module = "h/audit.ts",     entrypoint = "log",   key = "",          parallel = false }
]
```

The two parallel stages run concurrently. The audit stage runs sequentially after both complete, seeing `ctx.sources["pricing"]` and `ctx.sources["inventory"]`.
...
`Message::Binary` frames are logged as a warning and discarded. Rivers WebSocket views operate on text frames only. `Message::Ping` and `Message::Pong` are handled by the WS layer transparently.
...
Per-topic `tokio::sync::broadcast` channels managed by `TopicRegistry`. Topics are registered at server startup from config. Each topic has a configurable buffer size.
...
At deploy time, 6 rules are enforced against EventBus view configs:

1. Referenced topic must exist in `TopicRegistry`
...
6. SSE views with `sse_trigger_events` must reference valid topic names
...
on_request = [
    { module = "h/orders.ts", entrypoint = "fetchCustomer",    key = "customer",    parallel = true },
    { module = "h/orders.ts", entrypoint = "fetchShipping",    key = "shipping",    parallel = true },
    { module = "h/orders.ts", entrypoint = "fetchFulfillment", key = "fulfillment", parallel = true }
]
...
| EventBus topic in `sse_trigger_events` not registered | `unknown topic '{name}'` |
| `on_event.topic` not registered in TopicRegistry | `unknown topic '{name}'` |
| `rate_limit_per_minute = 0` | `rate_limit_per_minute must be greater than 0` |
| Parallel stage with `on_failure` set | `on_failure is only valid for transform stages` |
