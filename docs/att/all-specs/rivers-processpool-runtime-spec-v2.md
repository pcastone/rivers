...
- **Workers are isolated.** No state crosses between worker executions. A worker's V8 Isolate is reset (or recycled via a clean snapshot) after each task.
...
Isolate                              Host
──────────────────────────────────────────────────────
Rivers.db.query("tok:mysql-1",       →  resolve token → actual PgPool
...
Each V8 worker holds a single `v8::Isolate`. On worker startup, the isolate is created from a clean snapshot — the snapshot contains only the River standard library stubs, not a full Node.js or browser environment. The snapshot is pre-serialized and shared across workers for fast startup.
...
### 5.4 Isolate Reuse and Reset

After a task completes, the V8 worker performs a context reset rather than destroying and recreating the isolate. Context reset is faster than full isolate teardown:

1. The task context (global object) is replaced with a fresh template
2. The heap is not cleared (V8 manages heap through GC)
3. If heap usage after reset exceeds `max_heap_mb * 0.8`, the isolate is destroyed and recreated from the snapshot

This gives fast per-request reset with occasional full recycling for long-lived workers.
...
ProcessPool "default" startup:
  1. Load V8 isolate snapshot (shared across workers)
  2. Spawn N worker threads (config: workers = 4)
  3. Each worker thread: create Isolate from snapshot, enter idle state
  4. Spawn 1 watchdog thread for this pool
  5. Pool enters READY state — begins accepting tasks
```
...
The host validates the destination URL post-DNS resolution against RFC 1918 ranges before forwarding — SSRF protection is enforced at the host level regardless of the URL the handler passes.
...
| No SSRF from handlers | `Rivers.http` not injected unless declared; post-DNS RFC 1918 validation | SEC-11 |
...
|---|---|---|---|
| 1 | V8 snapshot content | What should the base snapshot include? Currently proposed: Rivers API stubs only. Alternative: include common libs (lodash, zod) in snapshot for faster load. | **Open** |
| 2 | Isolate-per-request vs reuse | Current design reuses isolates with context reset. Alternative: destroy and recreate per request for stronger isolation. Tradeoff is startup cost. | **Open** — default is reuse |
...
| 4 | Shared V8 Heap Snapshot | The V8 heap snapshot is shared read-only across workers. This is safe for immutable data but needs verification that no mutable state leaks across the snapshot boundary. | **Verify during implementation** |
