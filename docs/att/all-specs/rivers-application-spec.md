...
The primary application. Hosts the SPA and orchestrates app-services. Thin data layer — primarily curl datasources to app-services plus whatever datasources auth requires.

Characteristics:
- Serves SPA static assets
- Declares curl datasources (HTTP driver) pointing at app-services
- Owns the auth guard — session established here carries over to app-service calls
- Binds to a port or standard 80/443
...
app-bundle.zip
    ...
    │       ├─ SPA assets
    │       ├─ Auth guard
    │       └─ curl datasources → app-services (HTTP)
    │
...
| `entryPoint` | string | yes | Developer | Where riversd binds this app. Port or scheme determines binding behavior. |
...
### 5.3 entryPoint binding rules

| entryPoint value | riversd behavior |
|---|---|
| `http://0.0.0.0:{port}` | Binds HTTP on declared port |
| `https://0.0.0.0` | Binds HTTPS on 443. TLS config required in riversd.conf. |
| `https://0.0.0.0:{port}` | Binds HTTPS on declared port. |
| `http://0.0.0.0` | Binds HTTP on 80. |

Each port may be bound by exactly one app. If two apps declare the same port, the second deploy fails with a port conflict error.
...
When app-main declares a service dependency on `orders-service`:

```
resources.json: { "name": "orders-service", "appId": "f47ac10b-..." }
    │
    ▼
riversd resolves appId → appDeployId (at deploy time)
    │
    ▼
constructs service endpoint: http://{host}:{port}
    derived from orders-service entryPoint
    │
    ▼
registers curl datasource: "orders-service"
    base_url = resolved endpoint
    driver   = "http"
    │
    ▼
view declares: resources = ["orders-service"]
CodeComponent calls: Rivers.view.query("orders-service-endpoint", params)
```

### 7.2 Service name construction

The service endpoint name is constructed as `{appName}-srv`. For an app-service with `appName = "orders-service"`, the internal service identifier is `orders-service-srv`.

In v1, this resolves to a single HTTP endpoint on the app-service's declared `entryPoint` port running in the same riversd instance.

In v2 (RPS mode), `orders-service-srv` may resolve to a list of endpoints across nodes. riversd receives the list from RPS and load-balances across them. The app-main code is unchanged — the logical service name is the same.

### 7.3 App-service HTTP always

App-main to app-service communication is always HTTP — never in-process function calls. This holds even when both run in the same riversd instance. Reasons:

- Consistent behavior between v1 (single node) and v2 (multi-node RPS)
- App isolation — apps cannot share memory or call each other's internal code
- Failure isolation — an app-service crash does not crash app-main
- Observable — HTTP calls appear in traces, logs, and metrics

### 7.4 Unresolved service at deploy time

If `required = true` and the target `appId` is not deployed in this riversd, deployment of the dependent app fails:
...
If `required = false` and the target `appId` is not deployed, the app starts. Queries to that service return `null`. A `WARN` is logged at startup.
...
Before an app-service is considered RUNNING and before dependent app-mains are allowed to start, riversd performs a health check against the app-service's `entryPoint`:

```
GET {entryPoint}/health
```
...
| Port available | `entryPoint` port not already bound by another app |
...
| `entryPoint` port already bound | `port {port} is already bound by '{appName}'` |
