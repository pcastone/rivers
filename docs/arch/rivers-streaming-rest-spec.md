...
### 4.5 `Rivers.view.stream()`

`Rivers.view.stream()` invokes another streaming view registered in the same Rivers instance and returns an async generator of `StreamChunk` values. Use it when a streaming handler needs to consume or proxy the output of an internal streaming view.
...
`done` is `true` on the terminal chunk. The generator completes after yielding the terminal chunk. If the upstream view emits a poison chunk, the drive loop propagates the error downstream — the caller sees the generator throw before `done` is reached.

**LLM proxy example:**

```typescript
// handlers/llm-proxy.ts
export async function* proxyGenerate(
    req: Rivers.Request
): AsyncGenerator<any> {
    const { prompt, model } = req.body as { prompt: string; model: string };

    Rivers.log.info("proxying generation", { model, trace_id: req.trace_id });

    for await (const chunk of Rivers.view.stream<{ token?: string; done?: boolean }>(
...
The proxy yields each `chunk.data` directly to the client without transformation. The terminal `{ done: true }` is emitted by the proxy handler itself after the upstream generator completes.

### 4.6 Isolate reuse for streaming handlers

<!-- SHAPE-9 amendment: isolates reused with fresh per-request context -->
Streaming handlers follow the same isolate reuse model as non-streaming handlers: isolates are pooled and reused, with a fresh application context bound per request. For streaming handlers, the context is long-lived for the duration of the stream — it is created when the generator starts and destroyed (with zeroization) when the generator completes or is terminated. The security boundary is the context, not the isolate.

### 4.7 Pool config additions
...
This uses the standard `ErrorResponse` envelope (see HTTPD spec §18). <!-- SHAPE-2 amendment: pre-stream errors use ErrorResponse envelope -->
...
Then closes the connection. <!-- SHAPE-2 amendment: poison chunks are wire-format-only, not the standard ErrorResponse envelope -->

<!-- SHAPE-15 amendment: runtime blocks chunks containing reserved key -->
The `stream_terminated` field is the signal. Clients MUST inspect every chunk for this field. A valid non-error chunk from the application MUST NOT include `stream_terminated`. Rivers enforces this at runtime: if a yielded JSON object contains a top-level `stream_terminated` key, the chunk is blocked, a poison chunk is emitted, the generator is terminated, and the stream is closed. Handler code must strip or rename the key before yielding.
...
- **§6.1** — Pre-stream errors (before first yield) use the standard `ErrorResponse` envelope from HTTPD spec §18: `{code, message, details, trace_id}`
- **§6.2** — Post-stream poison chunks remain wire-format-specific (`{"error": "...", "error_type": "...", "stream_terminated": true}`). Poison chunks are NOT the standard ErrorResponse envelope — they are a streaming wire protocol construct. The distinction is intentional: once headers are committed (200 OK), the ErrorResponse envelope (which includes a `code` field) would be misleading.

### SHAPE-9: Isolate Reuse Model

- **§4.6** — Isolates (V8/WASM instances) are pooled and reused. The security boundary is the **context**, not the isolate. Context is created fresh per request and destroyed/zeroized after completion. Streaming handlers hold a long-lived context for the duration of the stream. Worker lifecycle: pick up task, bind context, execute, unbind context, return isolate to pool.

### SHAPE-15: `stream_terminated` Runtime Guard

- **§6.2** — If a yielded JSON object contains a top-level `stream_terminated` key, the chunk is blocked at runtime, a poison chunk is emitted, the generator is terminated, and the stream is closed. Handler code must strip or rename the key before yielding. This prevents accidental stream termination signals from application data.
