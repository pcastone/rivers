...
<!-- SHAPE-16 amendment: Retry-After parsing requires declared format -->
For 429 responses, Rivers checks for `Retry-After` header and parses it according to the declared `retry_after_format` (`"seconds"` or `"http_date"`). Only the declared format is parsed; failure to parse is ignored with a WARN log, and normal backoff applies. The parsed value is capped at `max_delay_ms`.
...
open_timeout_ms     = 30000    # how long circuit stays open (default: 30000)  <!-- SHAPE-1 amendment: renamed from open_duration_ms -->
...
<!-- SHAPE-1 amendment: open_duration_ms renamed to open_timeout_ms -->
[data.datasources.openai.circuit_breaker]
failure_threshold  = 5
window_ms          = 10000
...
on_request = [
    { module = "handlers/product.ts", entrypoint = "fetchReviews", key = "reviews" }  <!-- SHAPE-12 amendment: parallel removed; stages run sequentially -->,
    { module = "handlers/product.ts", entrypoint = "fetchInventory", key = "inventory" }  <!-- SHAPE-12 amendment: parallel removed; stages run sequentially -->
]
...
SQL DataView for core product data, two sequential HTTP DataView calls for enrichment. The handler pipeline collapses all three into one response. From the View layer's perspective, SQL and HTTP DataViews are identical.
