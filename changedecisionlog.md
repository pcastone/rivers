...
### POOL-CR-P0-3.2 — `ReleaseToken::release` made async

**File:** `crates/rivers-runtime/src/pool_handle.rs`, `crates/riversd/src/pool_handle_impl.rs`, `crates/rivers-runtime/src/dataview_engine.rs`
**Decision:** Initial design used a sync `release` method that internally `tokio::spawn`'d the async pool return. Task 9 integration test discovered that under `current_thread` runtime in tight sequential loops, the spawned release doesn't run before the next acquire, so the pool keeps creating new connections up to `max_size` instead of reusing — defeating the entire P0-3 motivation. Made the trait method `async`, dropped the spawn, await the release directly. Caller is always in async context (`DataViewExecutor::execute` / `execute_ddl`) so this is natural; performance is unchanged (`pool.release` was already a quick lock-and-push). The fire-and-forget property is lost, but it was an accidental side effect of the sync-trait limitation, not a feature.
**Spec reference:** `docs/code_review.md` finding P0-3; `rivers-data-layer-spec.md §5` (pool reuse contract).
**Resolution:** Trait now `#[async_trait] async fn release(self: Box<Self>, conn)`; impl awaits `pool.release(conn, Some(created_at))` directly; both `DataViewExecutor` call sites add `.await`. Reuse integration test now asserts `connect_count == 1` after 50 sequential dataview calls — perfect reuse, no yield workaround needed.

### POOL-CR-P0-3.1 — Concrete `PoolManager` on `AppContext`, trait at executor surface

**File:** `crates/riversd/src/server/context.rs`, `crates/rivers-runtime/src/pool_handle.rs`, `crates/rivers-runtime/src/dataview_engine.rs`
**Decision:** `AppContext.pool_manager` holds the concrete `Arc<PoolManager>` (not a trait object) so non-handler code (e.g. `bundle_loader::load.rs`'s `ensure_pool` calls) can use the inherent API directly. The executor side takes `Arc<dyn PoolManagerHandle>` (a small dyn-safe trait in `rivers-runtime`) so it doesn't need to depend on `riversd::pool` types — preserving the `rivers-runtime → riversd` dependency direction.
**Spec reference:** `docs/code_review.md` finding P0-3.
**Resolution:** Trait surface kept minimal (`acquire`, `snapshots`). Bridge impl lives in `crates/riversd/src/pool_handle_impl.rs` and is auto-discovered when `mod pool_handle_impl;` is declared in `lib.rs`. The `as Arc<dyn PoolManagerHandle>` coercion at the bundle-load wiring site is a one-liner.
